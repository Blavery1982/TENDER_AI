...
def _attach_decision(result: dict[str, Any], position: dict[str, Any]) -> dict[str, Any]:
    """Добавить решение только через общий детерминированный слой."""
    result["procurement"] = {
        "nmck": position.get("nmck"),
        "commission_fee": position.get("commission_fee"),
        "commission_source": position.get("commission_source"),
    }
    result["item"] = {
        "position_number": position.get("item_number", 1),
        "quantity": position.get("quantity"),
    }
    result["supplier_search"] = {
        "ranked_offers": result.get("ranked_offers") or [],
        "all_verified_offers": result.get("antifraud_history") or [],
    }
    result["audit"] = {"additional_expense_state": {
        "ready": False, "amount": None,
        "reason": "Специальные расходы в этом batch-пути ещё не подтверждены",
    }}
    ...
    for row in ready:
        ...
        try:
            ...
            result = build_price_search_result(position, price_result,
                                               verifier=verifier, eat_commission=position.get("commission_fee"))
        ...
        results.append(_attach_decision(result, position))
