...
def extract_direct_identifier(item: dict[str, Any]) -> dict[str, Any] | None:
    ...
    for field in ("model", "article", "sku", "customer_required_model"):
        ...
        if value:
            ...
            return {"identifier": value, "ambiguous": ambiguous, "reason": reason,
                    "source": "CUSTOMER_SPECIFICATION", "source_field": field}
    ...
    for candidate in candidates:
        ...
        if not ambiguous:
            return {"identifier": value, "ambiguous": False, "reason": None,
                    "source": "CUSTOMER_SPECIFICATION", "source_field": "position_text"}
        if reason and "похоже на тип или серию" in reason:
            ambiguous_candidate = {"identifier": value, "ambiguous": True, "reason": reason,
                                   "source": "CUSTOMER_SPECIFICATION", "source_field": "position_text"}
