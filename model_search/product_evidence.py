...
def manufacturer_model_in_text(text, model):
    ...
    extracted=extract_skus(target)
    if len(extracted)==1:target=extracted[0]
    ...
    return bool(re.search(pattern,source.upper()))
...
def exact_identifier_in_text(text, identifier):
    ...
    return bool(re.search(r'(?<![0-9a-zа-яё])'+re.escape(target)+r'(?![0-9a-zа-яё])',source,re.I))
...
def html_page(raw,url):
    ...
    extra={'canonical_url':scope.canonical}
    if scope.started:
...
def page_matches(page,sku):
    ...
    classic=len(extract_skus(sku))==1
    if not classic:
...
def public_offer(page,sku):
    ...
    if not offers:
        ...
        found=re.findall(r'(?im)^\s*цена\s*[:|]\s*(\d[\d \u00a0]*(?:[,.]\d{1,2})?)\s*(?:₽|руб\.?|RUB)\s*$',page.get('text',''))
        if len(found)==1:
